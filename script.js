...
window.loadGlobalChat = () => {
    ...
    window.currentChatRef.on('child_added', snap => {
        ...
        const _hideAdmin = window.session && window.session.role !== 'admin' && m && (m.id === 'admin' || (window.allUsersMapFull && window.allUsersMapFull[m.id] && window.allUsersMapFull[m.id].role === 'admin'));
