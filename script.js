...
// ==========================================
// BẢN VÁ v2: CÔNG TẮC MÀU PHỤNG VỤ
//  - Admin/Quản lý chỉnh -> ghi lên Firebase -> MỌI tài khoản đổi theo tức thì
//  - Học sinh / Giáo viên: chỉ xem, không tự đổi (tránh lệch màu giữa các máy)
//  - Không bao giờ tự reset về Hồng khi node Firebase còn trống
//  - Tự gắn lại listener khi Firebase khởi tạo lại (đăng nhập/đăng xuất)
// ==========================================
(() => {
  const PALETTE = {
  ...
  const VALID = Object.keys(PALETTE);
  const norm = (k) => (VALID.indexOf(k) !== -1 ? k : 'pink');
  ...
  const canEdit = () => {
    try {
      return typeof window.isAdminLevel === 'function' && window.isAdminLevel() === true && window.isDemoMode !== true;
  ...
  window.applyLiturgyColor = (key) => {
    ...
    if (lbl) {
      lbl.innerText = (window.liturgyAuto ? '🔄 Tự động: ' : '🎨 Thủ công: ') + p.name +
        (canEdit() ? '' : ' — do Ban Quản Trị cài đặt');
    }
  };

  window.refreshLiturgy = () => {
    window.liturgyManual = norm(window.liturgyManual);
    const key = window.liturgyAuto ? window.autoLiturgyColor() : window.liturgyManual;
    window.applyLiturgyColor(key);
    const editable = canEdit();
    const zone = document.getElementById('liturgy-manual-zone');
    if (zone) zone.style.opacity = (window.liturgyAuto || !editable) ? '0.45' : '1';
    document.querySelectorAll('.lit-swatch').forEach(b => { b.disabled = !!window.liturgyAuto || !editable; });
    const auto = document.getElementById('liturgy-auto-toggle');
    if (auto) { auto.checked = !!window.liturgyAuto; auto.disabled = !editable; }
  };

  // --- Khôi phục từ cache ngay lập tức (chạy cả khi chưa có Firebase / đang bảo trì) ---
  try {
    window.liturgyAuto = localStorage.getItem('liturgyAuto') === 'true';
    window.liturgyManual = norm(localStorage.getItem('liturgyManual'));
  ...
  const cacheLocal = () => {
    try {
      ...
      localStorage.setItem('liturgyManual', norm(window.liturgyManual));
    } catch (e) {}
  };

  // --- Lưu + đồng bộ lên Firebase (chỉ quản trị). Có hàng đợi nếu Firebase chưa sẵn sàng ---
  let pendingWrite = null;
  const flushWrite = () => {
    if (!pendingWrite || !window.db || !canEdit()) return;
    const payload = pendingWrite;
    pendingWrite = null;
    window.db.ref('config/liturgy').set(payload)
      .then(() => {
        if (typeof window.showToast === 'function') {
          try { window.showToast('Đã đồng bộ màu phụng vụ cho toàn bộ tài khoản ✓', true); } catch (e) {}
        }
      })
      .catch(err => {
        console.warn('[Liturgy] sync fail', err);
        pendingWrite = payload; // thử lại ở nhịp sau
        if (typeof window.showToast === 'function') {
          try { window.showToast('Chưa đồng bộ được màu — sẽ tự thử lại', false); } catch (e) {}
        }
      });
  };
  ...
  const persist = () => {
    ...
    pendingWrite = {
      ...
      color: norm(window.liturgyManual),
      updatedAt: typeof window.now === 'function' ? window.now() : Date.now(),
      updatedBy: (window.session && (window.session.name || window.session.uid)) || 'admin'
    };
    flushWrite();
  };
  setInterval(flushWrite, 3000); // tự thử lại khi mạng/Firebase trở lại

  // --- Sự kiện công tắc & ô màu (uỷ quyền nên không sợ DOM tạo sau) ---
  document.addEventListener('change', (ev) => {
    if (ev.target && ev.target.id === 'liturgy-auto-toggle') {
      if (!canEdit()) { ev.target.checked = !!window.liturgyAuto; return; }
  ...
  document.addEventListener('click', (ev) => {
    ...
    if (!sw || window.liturgyAuto || !canEdit()) return;
    window.liturgyManual = norm(sw.dataset.liturgy);
    window.refreshLiturgy();
    persist();
  });

  // --- Lắng nghe đồng bộ thời gian thực từ Firebase (tự gắn lại nếu db đổi) ---
  let boundDb = null;
  const attach = () => {
    if (!window.db || window.db === boundDb) return;
    boundDb = window.db;
    const ref = window.db.ref('config/liturgy');
    ref.on('value', s => {
      const v = s.val();
      if (!v || typeof v !== 'object') {
        // Node còn trống: GIỮ NGUYÊN màu hiện tại, admin sẽ tạo dữ liệu gốc
        if (canEdit()) persist();
        return;
      }
      window.liturgyAuto = v.auto === true;
      window.liturgyManual = norm(v.color);
      cacheLocal();
      window.refreshLiturgy();
    }, err => {
      console.warn('[Liturgy] listen fail', err);
      boundDb = null; // cho phép gắn lại
    });
  };
  attach();
  ...
  setInterval(() => { if (window.liturgyAuto) window.refreshLiturgy(); }, 10 * 60 * 1000);
...
/* =========================================================================
   PHẦN GỘP: NỘI DUNG CỦA patch.js (bản mới) — nạp sau nên ghi đè bản cũ
   Đã tách CSS sang style.css, không còn chèn <style> động.
   ========================================================================= */
/* =========================================================================
   PATCH.JS — XÂY LẠI TAB TÀI KHOẢN + TAB THÔNG BÁO (PHÂN LUỒNG VAI TRÒ)
   Nạp SAU script.js nên mọi hàm ở đây sẽ ghi đè bản cũ.
   ========================================================================= */
(function () {
  "use strict";
  ...
  const now = () => (typeof window.now === "function" ? window.now() : Date.now());
  const toast = (m, ok = true) => (typeof window.showToast === "function" ? window.showToast(m, ok) : console.log(m));
  const alertBox = (t, m, i) => (typeof window.showCustomAlert === "function" ? window.showCustomAlert(t, m, i) : alert(t + "\n" + m));
  const confirmBox = (t, m, cb) => (typeof window.showCustomConfirm === "function" ? window.showCustomConfirm(t, m, cb) : (confirm(t + "\n" + m) && cb()));
  const role = () => (window.session && window.session.role) || "hs";
  const isAdmin = () => role() === "admin" || window.isDemoMode === true;
  const isQl = () => role() === "ql";
  const isGv = () => role() === "gv";
  const fmtTime = (ms) => {
    if (!ms) return "--:--";
  ...
  const ROLE_TEXT = { admin: "QUẢN LÝ (BOSS)", ql: "QUẢN LÝ", gv: "GIÁO VIÊN", hs: "HỌC SINH", cuu_hs: "CỰU HỌC SINH" };
  const ROLE_COLOR = { admin: "#673AB7", ql: "#9C27B0", gv: "#1877F2", hs: "#4CAF50", cuu_hs: "#607D8B" };


  /* ------------------------------------------------------- MODAL ĐỘNG */
  function ensureModals() {
    if (document.getElementById("acc-sheet-modal")) return;
    const wrap = document.createElement("div");
    wrap.innerHTML = `
    <div id="acc-sheet-modal" class="hidden overlay-dark flex-center" style="z-index:100001;">
      <div class="modal-box shadow-lux fade-in" style="border-top:6px solid var(--pink);padding:24px 18px;border-radius:22px;position:relative;max-width:380px;width:92%;">
        <button onclick="window.toggleModal('acc-sheet-modal',false)" style="position:absolute;top:12px;right:14px;background:none;border:none;font-size:20px;color:var(--text-light);cursor:pointer;">&#10005;</button>
        <div id="acc-sheet-head" style="margin-bottom:18px;"></div>
        <div id="acc-sheet-actions" style="display:flex;flex-direction:column;gap:10px;"></div>
      </div>
    </div>
    <div id="acc-edit-modal" class="hidden overlay-dark flex-center" style="z-index:100002;">
      <div class="modal-box shadow-lux fade-in" style="border-top:6px solid #1877F2;padding:24px 18px;border-radius:22px;position:relative;max-width:380px;width:92%;">
        <button onclick="window.toggleModal('acc-edit-modal',false)" style="position:absolute;top:12px;right:14px;background:none;border:none;font-size:20px;color:var(--text-light);cursor:pointer;">&#10005;</button>
        <h3 style="color:#1877F2;font-weight:900;margin-bottom:14px;">SỬA THÔNG TIN</h3>
        <input type="hidden" id="acc-edit-uid">
        <input type="text" id="acc-edit-name" placeholder="Tên hiển thị" style="width:100%;padding:12px;border-radius:12px;border:1px solid var(--border);background:var(--bg);color:var(--text);margin-bottom:10px;">
        <select id="acc-edit-role" style="width:100%;padding:12px;border-radius:12px;border:1px solid var(--border);background:var(--bg);color:var(--text);margin-bottom:14px;"></select>
        <button class="acc-sheet-btn" style="background:#1877F2;" onclick="window.saveAccountEdit()">LƯU THAY ĐỔI</button>
      </div>
    </div>
    <div id="acc-pass-modal" class="hidden overlay-dark flex-center" style="z-index:100002;">
      <div class="modal-box shadow-lux fade-in" style="border-top:6px solid #9C27B0;padding:24px 18px;border-radius:22px;position:relative;max-width:380px;width:92%;">
        <button onclick="window.toggleModal('acc-pass-modal',false)" style="position:absolute;top:12px;right:14px;background:none;border:none;font-size:20px;color:var(--text-light);cursor:pointer;">&#10005;</button>
        <h3 style="color:#9C27B0;font-weight:900;margin-bottom:6px;">ĐỔI MẬT KHẨU</h3>
        <p id="acc-pass-name" style="font-size:13px;color:var(--text-light);font-weight:bold;margin-bottom:14px;"></p>
        <input type="hidden" id="acc-pass-uid">
        <input type="hidden" id="acc-pass-old">
        <input type="text" id="acc-pass-new" placeholder="Mật khẩu mới (tối thiểu 6 ký tự)" style="width:100%;padding:12px;border-radius:12px;border:1px solid var(--border);background:var(--bg);color:var(--text);margin-bottom:14px;">
        <button class="acc-sheet-btn" style="background:#9C27B0;" onclick="window.saveAccountPassword()">ĐỔI MẬT KHẨU</button>
      </div>
    </div>`;
  ...
  /* =====================================================================
     ...
     - admin : toàn quyền, là người DUY NHẤT được XOÁ tài khoản
     - ql    : tạo / sửa / khoá mọi tài khoản TRỪ admin, không được xoá
     - gv    : chỉ được KHOÁ / MỞ KHOÁ học sinh, tạo tài khoản học sinh
     - hs    : không thấy tab này
     ===================================================================== */
  const canSeeAccountTab = () => isAdmin() || isQl() || isGv();
  const canCreate = () => isAdmin() || isQl() || isGv();
  const creatableRoles = () => (isAdmin() ? ["hs", "gv", "ql"] : isQl() ? ["hs", "gv", "ql"] : ["hs"]);
  const canTouch = (targetRole, targetId) => {
    if (targetId === (window.session && window.session.id)) return false;
    if (targetRole === "admin" || targetId === "admin") return isAdmin();
    if (isAdmin() || isQl()) return true;
    if (isGv()) return targetRole === "hs" || targetRole === "cuu_hs";
    return false;
  };
  const canEditInfo = (tr, ti) => canTouch(tr, ti) && (isAdmin() || isQl());
  const canLock = (tr, ti) => canTouch(tr, ti);
  const canDelete = (tr, ti) => isAdmin() && ti !== "admin" && tr !== "admin";
  const canSeeLockLog = () => isAdmin() || isQl();

  /* --------------------------------------------------------- TAB TÀI KHOẢN */
  window.__accFilter = "all";
  window.__accSearch = "";

  window.setAccountFilter = (f) => {
    window.__accFilter = f;
    document.querySelectorAll("#acc-filter-bar .acc-chip").forEach((c) => c.classList.toggle("active", c.dataset.filter === f));
    renderAccounts();
  };
  window.searchStudent = () => {
    const el = document.getElementById("search-user");
    window.__accSearch = el ? el.value.trim().toLowerCase() : "";
    renderAccounts();
  };
  ...
  function fillCreateRoleSelect() {
    ...
    if (!sel) return;
    sel.innerHTML = creatableRoles().map((r) => `<option value="${r}">${ROLE_TEXT[r]}</option>`).join("");
    const card = document.getElementById("acc-create-card");
    if (card) card.classList.toggle("hidden", !canCreate());
  }

  /* TỰ CHỮA GIAO DIỆN: nếu HTML trên host là bản CŨ (chưa có acc-list-staff/
     acc-list-student, vì trước đây dùng bảng list-gv/list-hs), thì dựng lại
     khung danh sách ngay trong #tab-users để tab Tài khoản không bị trắng. */
  function ensureAccountTabShell() {
    if (document.getElementById("acc-list-staff") && document.getElementById("acc-list-student")) return true;
    const tab = document.getElementById("tab-users") || document.getElementById("tab-manage");
    if (!tab) return false;
    const box = document.createElement("div");
    box.id = "acc-selfheal-wrap";
    box.innerHTML = `
      <div id="acc-filter-bar" style="display:flex;gap:6px;flex-wrap:wrap;margin:10px 0 15px;">
        <span class="acc-chip active" data-filter="all" onclick="window.setAccountFilter('all')">Tất cả</span>
        <span class="acc-chip" data-filter="gv" onclick="window.setAccountFilter('gv')">Giáo viên</span>
        <span class="acc-chip" data-filter="ql" onclick="window.setAccountFilter('ql')">Quản lý</span>
        <span class="acc-chip" data-filter="hs" onclick="window.setAccountFilter('hs')">Học sinh</span>
        <span class="acc-chip" data-filter="locked" onclick="window.setAccountFilter('locked')">Đang khoá</span>
      </div>
      <div class="card shadow-lux mb-15">
        <h4 style="color:var(--pink);margin-bottom:10px;">GIÁO VIÊN / QUẢN LÝ</h4>
        <div id="acc-list-staff" style="display:flex;flex-direction:column;gap:10px;"></div>
      </div>
      <div class="card shadow-lux mb-15">
        <h4 style="color:var(--pink);margin-bottom:10px;">HỌC SINH</h4>
        <div id="acc-list-student" style="display:flex;flex-direction:column;gap:10px;"></div>
      </div>`;
    tab.appendChild(box);
    return true;
  }
  ...
  function renderAccounts() {
    ensureAccountTabShell();
    const staffEl = document.getElementById("acc-list-staff");
    const hsEl = document.getElementById("acc-list-student");
    if (!staffEl || !hsEl) return;

    const users = window.__accUsers || {};
    const pass = window.__accPass || {};
    let staff = "", hs = "", total = 0, nStaff = 0, nHs = 0, nLocked = 0;

    Object.keys(users).sort().forEach((uid) => {
      const u = users[uid] || {};
      const r = u.role || "hs";
      if (!isAdmin() && (uid === "admin" || r === "admin")) return;
      total++;
      if (r === "gv" || r === "ql" || r === "admin") nStaff++; else nHs++;
      if (u.isLocked) nLocked++;

      const f = window.__accFilter;
      if (f === "locked" && !u.isLocked) return;
      if (f !== "all" && f !== "locked" && r !== f) return;
      const q = window.__accSearch;
      if (q && !(uid.toLowerCase().includes(q) || String(u.name || "").toLowerCase().includes(q))) return;

      const showPass = isAdmin() || isQl() || (isGv() && r === "hs");
      const pv = showPass && pass[uid] && pass[uid].pass ? pass[uid].pass : "••••••";
      const avt = u.avatar || "https://cdn-icons-png.flaticon.com/512/149/149071.png";
      const lockBadge = u.isLocked ? `<span class="acc-badge" style="background:#FF9800;">🔒 ĐÃ KHOÁ</span>` : "";
      const row = `
      <div class="acc-row">
        <img src="${avt}" alt="">
        <div class="acc-main">
          <div style="font-weight:900;color:var(--pink);font-size:14px;">${esc(u.name || uid)}
            <span class="acc-badge" style="background:${ROLE_COLOR[r] || "#607D8B"};">${ROLE_TEXT[r] || r}</span>${lockBadge}</div>
          <div style="font-size:11px;color:var(--text-light);font-weight:bold;margin-top:3px;">ID: ${esc(uid.toUpperCase())} • MK: ${esc(pv)}</div>
          ${u.isLocked && u.lockReason ? `<div style="font-size:11px;color:#FF9800;margin-top:3px;">Lý do: ${esc(u.lockReason)}</div>` : ""}
        </div>
        <button class="acc-act" onclick="window.openUserActionMenu('${esc(uid)}')"><i class="fas fa-ellipsis-v"></i></button>
      </div>`;
      if (r === "gv" || r === "ql" || r === "admin") staff += row; else hs += row;
    });

    staffEl.innerHTML = staff || `<div class="acc-empty">Không có tài khoản phù hợp.</div>`;
    hsEl.innerHTML = hs || `<div class="acc-empty">Không có tài khoản phù hợp.</div>`;
    const set = (id, v) => { const e = document.getElementById(id); if (e) e.innerText = v; };
    set("acc-stat-total", total); set("acc-stat-staff", nStaff); set("acc-stat-hs", nHs); set("acc-stat-locked", nLocked);
  }

  window.loadUsers = () => {
    // DB chưa sẵn sàng (Firebase còn đang khởi tạo / mất mạng): thử lại thay vì
    // im lặng bỏ qua — đây là lý do tab Tài khoản trắng trơn khi vào app nhanh.
    if (!window.db) {
      if (window.__loadUsersRetry) clearTimeout(window.__loadUsersRetry);
      window.__loadUsersTries = (window.__loadUsersTries || 0) + 1;
      if (window.__loadUsersTries <= 60) {
        window.__loadUsersRetry = setTimeout(() => window.loadUsers(), 400);
      }
      return;
    }
    window.__loadUsersTries = 0;
    ensureModals();
    ensureAccountTabShell();
    fillCreateRoleSelect();
    document.querySelectorAll("#acc-filter-bar .acc-chip").forEach((c) => c.classList.toggle("active", c.dataset.filter === window.__accFilter));
    ...
    window._usersRef.on("value", (s) => {
      const raw = s.val() || {};
      window.allUsersMapFull = raw;
      const visible = {};
      Object.keys(raw).forEach((uid) => {
        if (!isAdmin() && (uid === "admin" || (raw[uid] && raw[uid].role === "admin"))) return;
        visible[uid] = raw[uid];
      });
      window.allUsersMap = visible;
      window.__accUsers = visible;
      renderAccounts();
    }, (err) => {
      // Firebase chặn đọc (Rules / chưa đăng nhập) — báo rõ thay vì để trắng tab
      console.error("[Accounts] không đọc được /users:", err);
      const staffEl = document.getElementById("acc-list-staff");
      const hsEl = document.getElementById("acc-list-student");
      const msg = `<div class="acc-empty">⚠️ Không tải được dữ liệu từ Firebase (${esc(err && err.message ? err.message : "permission denied")}). Kiểm tra Rules mục /users.</div>`;
      if (staffEl) staffEl.innerHTML = msg;
      if (hsEl) hsEl.innerHTML = msg;
    });

    if (isAdmin() || isQl() || isGv()) {
      window._userPassRef = window.db.ref("user_passwords");
      window._userPassRef.on("value", (p) => { window.__accPass = p.val() || {}; renderAccounts(); },
        (err) => { console.warn("[Accounts] không đọc được /user_passwords:", err); window.__accPass = {}; renderAccounts(); });
    }
    if (canSeeLockLog()) loadLockLogs();
  };

  /* ------------------------------------------------------------- TẠO MỚI */
  window.createNewUser = () => {
    if (!canCreate()) return alertBox("LỖI QUYỀN", "Bạn không có quyền tạo tài khoản!", "🚫");
    const idEl = document.getElementById("new-id"), nEl = document.getElementById("new-name"), pEl = document.getElementById("new-pass");
    const id = (idEl.value || "").toLowerCase().trim().replace(/\s+/g, "");
    const n = (nEl.value || "").trim();
    const p = (pEl.value || "").trim();
    let r = document.getElementById("new-role").value;
    if (!creatableRoles().includes(r)) r = "hs";
    if (!id || !n || !p) return alertBox("THIẾU THÔNG TIN", "Vui lòng điền đủ ID, tên và mật khẩu!", "⚠️");
    if (!/^[a-z0-9._-]+$/.test(id)) return alertBox("ID KHÔNG HỢP LỆ", "ID chỉ gồm chữ thường, số và . _ -", "⚠️");
    if (p.length < 6) return alertBox("MẬT KHẨU YẾU", "Mật khẩu phải từ 6 ký tự!", "⚠️");

    window.db.ref("users/" + id).once("value").then((snap) => {
      if (snap.exists()) return alertBox("TRÙNG ID", `ID '${id}' đã có người sử dụng!`, "❌");
      const app2 = firebase.apps.length > 1 ? firebase.app("App2") : firebase.initializeApp(firebase.app().options, "App2");
      app2.auth().createUserWithEmailAndPassword(id + "@kimminlai.com", p).then(() => {
        app2.auth().signOut();
        window.db.ref("users/" + id).set({ name: n, role: r, isLocked: false, allowPrivate: true }).then(() => {
          ...
          alertBox("THÀNH CÔNG", `Đã tạo tài khoản ${n} (${ROLE_TEXT[r]})!`, "✅");
        });
      }).catch((e) => alertBox("LỖI", e.message, "❌"));
    });
  };

  /* ------------------------------------------------------- MENU THAO TÁC */
  window.openUserActionMenu = (uid) => {
    ensureModals();
    const u = (window.__accUsers && window.__accUsers[uid]) || (window.allUsersMapFull && window.allUsersMapFull[uid]) || {};
    const r = u.role || "hs";
    const head = document.getElementById("acc-sheet-head");
    const acts = document.getElementById("acc-sheet-actions");
    if (!head || !acts) return;

    head.innerHTML = `
      <div style="text-align:center;">
        <img src="${u.avatar || "https://cdn-icons-png.flaticon.com/512/149/149071.png"}" style="width:64px;height:64px;border-radius:50%;object-fit:cover;border:2px solid var(--pink);">
        <div style="font-weight:900;color:var(--pink);font-size:16px;margin-top:8px;">${esc(u.name || uid)}</div>
        <div style="font-size:12px;color:var(--text-light);font-weight:bold;">ID: ${esc(uid.toUpperCase())} • ${ROLE_TEXT[r] || r}</div>
        ${u.isLocked ? `<div style="font-size:12px;color:#FF9800;font-weight:bold;margin-top:5px;">🔒 Đang bị khoá — ${esc(u.lockReason || "Không rõ lý do")}</div>` : ""}
      </div>`;

    let html = "";
    if (canEditInfo(r, uid)) {
      html += `<button class="acc-sheet-btn" style="background:#1877F2;" onclick="window.openAccountEdit('${esc(uid)}')">✏️ Sửa tên &amp; vai trò</button>`;
      html += `<button class="acc-sheet-btn" style="background:#9C27B0;" onclick="window.openAccountPassword('${esc(uid)}')">🔑 Đổi mật khẩu</button>`;
    }
    if (canLock(r, uid)) {
      html += u.isLocked
        ? `<button class="acc-sheet-btn" style="background:#4CAF50;" onclick="window.clickToggleLock('${esc(uid)}','${esc(u.name || uid)}',true)">🔓 Mở khoá tài khoản</button>`
        : `<button class="acc-sheet-btn" style="background:#FF9800;" onclick="window.clickToggleLock('${esc(uid)}','${esc(u.name || uid)}',false)">🔒 Khoá tài khoản</button>`;
    }
    if (canDelete(r, uid)) {
      html += `<button class="acc-sheet-btn" style="background:#dc3545;" onclick="window.clickDelete('${esc(uid)}','${esc(u.name || uid)}')">🗑️ Xoá vĩnh viễn</button>`;
    }
    if (!html) html = `<div class="acc-empty">Bạn không có quyền thao tác với tài khoản này.</div>`;
    acts.innerHTML = html;
    window.toggleModal("acc-sheet-modal", true);
  };

  window.openAccountEdit = (uid) => {
    const u = (window.__accUsers && window.__accUsers[uid]) || {};
    window.toggleModal("acc-sheet-modal", false);
    document.getElementById("acc-edit-uid").value = uid;
    document.getElementById("acc-edit-name").value = u.name || "";
    const roles = isAdmin() ? ["hs", "cuu_hs", "gv", "ql"] : ["hs", "cuu_hs", "gv", "ql"];
    document.getElementById("acc-edit-role").innerHTML = roles.map((r) => `<option value="${r}" ${r === (u.role || "hs") ? "selected" : ""}>${ROLE_TEXT[r]}</option>`).join("");
    window.toggleModal("acc-edit-modal", true);
  };

  window.saveAccountEdit = () => {
    const uid = document.getElementById("acc-edit-uid").value;
    const name = document.getElementById("acc-edit-name").value.trim();
    const newRole = document.getElementById("acc-edit-role").value;
    const u = (window.__accUsers && window.__accUsers[uid]) || {};
    if (!canEditInfo(u.role || "hs", uid)) return alertBox("LỖI QUYỀN", "Hành động bị từ chối!", "🚫");
    if (!name) return alertBox("THIẾU TÊN", "Tên hiển thị không được để trống!", "⚠️");
    window.db.ref("users/" + uid).update({ name, role: newRole }).then(() => {
      window.toggleModal("acc-edit-modal", false);
      toast("✅ Đã cập nhật tài khoản!");
    }).catch((e) => alertBox("LỖI", e.message, "❌"));
  };

  window.openAccountPassword = (uid) => {
    const u = (window.__accUsers && window.__accUsers[uid]) || {};
    window.toggleModal("acc-sheet-modal", false);
    document.getElementById("acc-pass-uid").value = uid;
    document.getElementById("acc-pass-old").value = (window.__accPass && window.__accPass[uid] && window.__accPass[uid].pass) || "";
    document.getElementById("acc-pass-name").innerText = (u.name || uid) + " (" + uid.toUpperCase() + ")";
    document.getElementById("acc-pass-new").value = "";
    window.toggleModal("acc-pass-modal", true);
  };

  window.saveAccountPassword = () => {
    const uid = document.getElementById("acc-pass-uid").value;
    const oldPass = document.getElementById("acc-pass-old").value;
    const newPass = document.getElementById("acc-pass-new").value.trim();
    const u = (window.__accUsers && window.__accUsers[uid]) || {};
    if (!canEditInfo(u.role || "hs", uid)) return alertBox("LỖI QUYỀN", "Hành động bị từ chối!", "🚫");
    if (newPass.length < 6) return alertBox("MẬT KHẨU YẾU", "Mật khẩu mới phải từ 6 ký tự!", "⚠️");
    if (!oldPass) return alertBox("KHÔNG ĐỔI ĐƯỢC", "Hệ thống không lưu mật khẩu cũ của tài khoản này.", "⚠️");
    const app2 = firebase.apps.length > 1 ? firebase.app("App2") : firebase.initializeApp(firebase.app().options, "App2");
    app2.auth().signInWithEmailAndPassword(uid + "@kimminlai.com", oldPass).then((cred) =>
      cred.user.updatePassword(newPass).then(() => {
        app2.auth().signOut();
        return window.db.ref("user_passwords/" + uid).update({ pass: newPass });
      })
    ).then(() => {
      window.toggleModal("acc-pass-modal", false);
      toast("✅ Đã đổi mật khẩu!");
    }).catch((e) => { try { app2.auth().signOut(); } catch (_) {} alertBox("LỖI", e.message, "❌"); });
  };

  /* ------------------------------------------------------- KHOÁ / MỞ KHOÁ */
  window.clickToggleLock = (uid, name, isLocked) => {
    window.toggleModal("acc-sheet-modal", false);
    const u = (window.__accUsers && window.__accUsers[uid]) || {};
    if (!canLock(u.role || "hs", uid)) return alertBox("LỖI QUYỀN", "Hành động bị từ chối!", "🚫");
    if (isLocked) {
      window.db.ref("users/" + uid).update({ isLocked: false, lockReason: null }).then(() => {
        writeLockLog(uid, name, "unlock", "");
        toast("🔓 Đã mở khoá " + name);
      });
      return;
    }
    document.getElementById("lock-u-id").value = uid;
  ...
  window.confirmLockUser = () => {
    const uid = document.getElementById("lock-u-id").value;
    const u = (window.__accUsers && window.__accUsers[uid]) || {};
    if (!canLock(u.role || "hs", uid)) return alertBox("LỖI QUYỀN", "Hành động bị từ chối!", "🚫");
    const reason = document.getElementById("lock-reason-input").value.trim() || "Vi phạm nội quy";
    window.db.ref("users/" + uid).update({ isLocked: true, lockReason: reason }).then(() => {
      writeLockLog(uid, u.name || uid, "lock", reason);
      window.toggleModal("lock-reason-modal", false);
      toast("🔒 Đã khoá tài khoản " + (u.name || uid));
    }).catch((e) => alertBox("LỖI", e.message, "❌"));
  };

  function writeLockLog(uid, name, action, reason) {
    if (!window.db) return;
    window.db.ref("lock_logs").push().set({
      uid, name: name || uid, action, reason: reason || "",
      by: (window.session && window.session.id) || "?",
      byName: (window.session && window.session.name) || "?",
      byRole: role(), time: now(),
    }).catch(() => {});
  }

  function loadLockLogs() {
    const card = document.getElementById("acc-lock-log-card");
    const list = document.getElementById("acc-lock-log-list");
    if (!card || !list) return;
    card.classList.remove("hidden");
    if (window._lockLogRef) { try { window._lockLogRef.off(); } catch (e) {} }
    window._lockLogRef = window.db.ref("lock_logs").limitToLast(100);
    window._lockLogRef.on("value", (snap) => {
      const rows = [];
      snap.forEach((c) => rows.push({ key: c.key, ...c.val() }));
      rows.reverse();
      list.innerHTML = rows.length ? rows.map((l) => `
        <div style="display:flex;align-items:center;gap:10px;padding:10px;border-radius:12px;background:var(--card);border:1px solid var(--border);">
          <div style="flex:1;min-width:0;">
            <div style="font-size:13px;font-weight:900;color:${l.action === "lock" ? "#FF9800" : "#4CAF50"};">
              ${l.action === "lock" ? "🔒 KHOÁ" : "🔓 MỞ KHOÁ"} — ${esc(l.name)} <span style="color:var(--text-light);font-weight:bold;font-size:11px;">(${esc(String(l.uid).toUpperCase())})</span>
            </div>
            <div style="font-size:12px;color:var(--text);margin-top:3px;">Người thực hiện: <b>${esc(l.byName)}</b> (${ROLE_TEXT[l.byRole] || esc(l.byRole || "")})</div>
            ${l.reason ? `<div style="font-size:12px;color:var(--text-light);margin-top:2px;">Lý do: ${esc(l.reason)}</div>` : ""}
            <div style="font-size:11px;color:var(--text-light);margin-top:2px;">${fmtTime(l.time)}</div>
          </div>
          <button class="acc-act" style="color:#dc3545;" onclick="window.deleteLockLog('${l.key}')"><i class="fas fa-trash-alt"></i></button>
        </div>`).join("") : `<div class="acc-empty">Chưa có nhật ký khoá tài khoản nào.</div>`;
    });
  }

  window.deleteLockLog = (key) => {
    if (!canSeeLockLog()) return;
    confirmBox("XOÁ DÒNG NHẬT KÝ", "Xoá dòng nhật ký này khỏi bảng theo dõi?", () => {
      window.db.ref("lock_logs/" + key).remove().then(() => toast("🗑️ Đã xoá dòng nhật ký."));
    });
  };

  window.clearLockLogs = () => {
    if (!canSeeLockLog()) return;
    confirmBox("DỌN SẠCH NHẬT KÝ", "Xoá toàn bộ nhật ký khoá tài khoản? Không thể hoàn tác.", () => {
      window.db.ref("lock_logs").remove().then(() => toast("✅ Đã dọn sạch nhật ký."));
    });
  };

  /* -------------------------------------------------------------- XOÁ TK */
  const oldClickDelete = window.clickDelete;
  window.clickDelete = (uid, name) => {
    window.toggleModal("acc-sheet-modal", false);
    const u = (window.__accUsers && window.__accUsers[uid]) || {};
    if (!canDelete(u.role || "hs", uid)) return alertBox("LỖI QUYỀN", "Chỉ Admin mới được xoá tài khoản!", "🚫");
    if (typeof oldClickDelete === "function") return oldClickDelete(uid, name);
  };
  const oldConfirmDelete = window.confirmDeleteUser;
  window.confirmDeleteUser = function () {
    if (!isAdmin()) return alertBox("LỖI QUYỀN", "Chỉ Admin mới được xoá tài khoản!", "🚫");
    return oldConfirmDelete.apply(this, arguments);
  };

  /* =====================================================================
     TAB THÔNG BÁO — PHÂN LUỒNG GỬI THEO VAI TRÒ
     hs    : chỉ xem
     gv    : gửi cho Học sinh, Quản lý
     ql    : gửi cho Tất cả / Học sinh / Giáo viên (không gửi Admin)
     admin : gửi cho tất cả nhóm
     ===================================================================== */
  const TARGETS = { all: "🌐 Tất cả", hs: "🎓 Học sinh", gv: "👨‍🏫 Giáo viên", ql: "🧑‍💼 Quản lý", admin: "👑 Admin" };
  function allowedTargets() {
    if (isAdmin()) return ["all", "hs", "gv", "ql", "admin"];
    if (isQl()) return ["all", "hs", "gv"];
    if (isGv()) return ["hs", "ql"];
    return [];
  }

  function setupComposer() {
    const zone = document.getElementById("rules-editor-zone");
    const note = document.getElementById("rules-readonly-note");
    const sel = document.getElementById("announce-target");
    const hint = document.getElementById("rules-editor-hint");
    if (!zone || !sel) return;
    ...
    zone.classList.remove("hidden");
    sel.innerHTML = t.map((k) => `<option value="${k}">${TARGETS[k]}</option>`).join("");
    if (hint) {
      hint.innerText = isAdmin() ? "Bạn gửi được cho tất cả mọi người."
        : isQl() ? "Bạn gửi được cho tất cả trừ Admin."
        : "Bạn gửi được cho Học sinh và Quản lý.";
    }
  }
  ...
  window.saveAnnouncement = () => {
    ...
    const sel = document.getElementById("announce-target");
    if (!input) return;
    if (!allowedTargets().length) return alertBox("LỖI QUYỀN", "Bạn chỉ có quyền xem thông báo!", "🚫");
    const content = input.value.trim();
    if (!content) return alertBox("NHẮC NHỞ", "Vui lòng nhập nội dung thông báo!", "✏️");
    if (!window.db) return alertBox("LỖI KẾT NỐI", "Cơ sở dữ liệu chưa sẵn sàng!", "❌");
    let target = sel ? sel.value : "all";
    if (!allowedTargets().includes(target)) target = allowedTargets()[0];

    window.db.ref("announcements").push().set({
      content, target, timestamp: now(),
      sender: (window.session && window.session.name) || "Ban Quản Trị",
      senderId: (window.session && window.session.id) || "",
      senderRole: role(),
      role: role(),
    }).then(() => { input.value = ""; toast("🚀 Đã đăng thông báo!"); })
      .catch((e) => alertBox("LỖI", e.message, "❌"));
  };

  window.__annFilter = "all";
  window.setAnnouncementFilter = (f) => {
    window.__annFilter = f;
    document.querySelectorAll("#ann-filter-bar .acc-chip").forEach((c) => c.classList.toggle("active", c.dataset.filter === f));
    if (typeof window.__renderAnn === "function") window.__renderAnn();
  };

  window.loadAnnouncements = () => {
    const display = document.getElementById("rules-display");
    if (!display || !window.db) return;
    setupComposer();

    let bar = document.getElementById("ann-filter-bar");
    if (!bar) {
      bar = document.createElement("div");
      bar.id = "ann-filter-bar";
      bar.style.cssText = "display:flex;gap:6px;margin-bottom:12px;flex-wrap:wrap;";
      bar.innerHTML = [["all", "Tất cả"], ["admin", "👑 Admin"], ["ql", "🧑‍💼 Quản lý"], ["gv", "👨‍🏫 Giáo viên"], ["system", "⚙️ Hệ thống"]]
        .map(([k, l]) => `<button class="acc-chip ${k === "all" ? "active" : ""}" data-filter="${k}" onclick="window.setAnnouncementFilter('${k}')">${l}</button>`).join("");
      display.parentNode.insertBefore(bar, display);
    }

    if (window._annRef) { try { window._annRef.off(); } catch (e) {} }
    window._annRef = window.db.ref("announcements");
    window._annRef.on("value", (snap) => {
      const data = snap.val() || {};
      const list = Object.keys(data).map((k) => ({ id: k, ...data[k] })).sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
      const myRole = role();
      const myId = (window.session && window.session.id) || "";
      ...
      window.__renderAnn = () => {
        ...
        list.forEach((item) => {
          const target = item.target || "all";
          const senderRole = (item.senderRole || item.role || "system").toLowerCase();
          // Ai được xem
          const visible = isAdmin() || target === "all" || target === myRole || target === myId || item.senderId === myId;
          if (!visible) return;
          if (filt !== "all" && senderRole !== filt) return;

          const canRemove = isAdmin() || (isQl() && senderRole !== "admin") || item.senderId === myId;
          const srcColor = ROLE_COLOR[senderRole] || "#607D8B";
          const srcLabel = ROLE_TEXT[senderRole] || "HỆ THỐNG";
          html += `
          <div class="card" style="border:1px solid var(--border);padding:12px;border-radius:12px;background:var(--card);display:flex;flex-direction:column;gap:6px;">
            <div style="display:flex;justify-content:space-between;align-items:center;gap:6px;flex-wrap:wrap;">
              <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;">
                <span class="acc-badge" style="background:${srcColor};margin-left:0;">${srcLabel}</span>
                <span style="font-weight:900;color:var(--pink);font-size:13px;">📣 ${esc(item.sender || "Ban Quản Trị")}</span>
                <span class="acc-badge" style="background:var(--soft);color:var(--pink);">${TARGETS[target] || "✉️ " + esc(String(target).toUpperCase())}</span>
              </div>
              <div style="display:flex;align-items:center;gap:6px;">
                <span style="font-size:11px;color:var(--text-light);font-weight:bold;">${fmtTime(item.timestamp)}</span>
                ${canRemove ? `<button onclick="window.deleteAnnouncement('${item.id}')" style="background:none;border:none;color:var(--pink);cursor:pointer;font-size:13px;"><i class="fas fa-trash-alt"></i></button>` : ""}
              </div>
            </div>
            <div style="font-size:14px;color:var(--text);line-height:1.5;white-space:pre-line;word-break:break-word;">${esc(item.content || "")}</div>
          </div>`;
        });
        display.innerHTML = html || `<div class="acc-empty">📢 Không có thông báo phù hợp bộ lọc.</div>`;
      };
      window.__renderAnn();
    });
  };
  ...
  window.deleteAnnouncement = (id) => {
    if (role() === "hs") return alertBox("LỖI QUYỀN", "Học sinh không được gỡ thông báo!", "🚫");
    confirmBox("GỠ THÔNG BÁO", "Bạn chắc chắn muốn gỡ thông báo này?", () => {
      window.db.ref("announcements/" + id).remove()
        .then(() => toast("🗑️ Đã gỡ thông báo!"))
        .catch((e) => alertBox("THẤT BẠI", e.message, "⚠️"));
    });
  };

  /* =====================================================================
     TAB THEO DÕI — nút LÀM MỚI nạp lại đủ 3 bảng và báo kết quả rõ ràng
     ===================================================================== */
  window.refreshTracking = () => {
    const btn = document.getElementById("btn-refresh-track");
    if (!window.db) return alertBox("LỖI KẾT NỐI", "Cơ sở dữ liệu chưa sẵn sàng!", "❌");
    ...
    Promise.all([
      ...
      window.db.ref("grade_logs").limitToLast(50).once("value"),
      window.db.ref("inbox").once("value"),
    ]).then(([tSnap]) => {
      if (typeof window.renderTrackingTable === "function") window.renderTrackingTable(tSnap);
      let online = 0;
      const d = tSnap.val() || {};
      Object.keys(d).forEach((k) => { if (d[k] && d[k].status === "online") online++; });
      toast(`✅ Đã cập nhật — ${online} người đang online`);
    }).catch((e) => alertBox("LỖI", e.message, "❌"))
      .finally(() => { if (btn) { btn.innerHTML = "🔄 LÀM MỚI"; btn.disabled = false; } });
  };

  /* ------------------------------------------------- KÍCH HOẠT SAU KHI VÀO */
  const oldEnterApp = window.enterApp;
  window.enterApp = function () {
    const out = oldEnterApp ? oldEnterApp.apply(this, arguments) : undefined;
    try {
      ensureModals();
      fillCreateRoleSelect();
      setupComposer();
      window.loadAnnouncements();
      if (canSeeAccountTab()) window.loadUsers();
      const prevTab = window.switchTab;
      if (!window.__accTabHooked) {
        window.__accTabHooked = true;
        window.switchTab = (id) => {
          if (typeof prevTab === "function") prevTab(id);
          if (id === "users" && canSeeAccountTab()) window.loadUsers();
        };
      }
      const navUsers = document.getElementById("nav-users");
      if (navUsers && !canSeeAccountTab()) navUsers.classList.add("hidden");
    } catch (e) { console.warn("[patch] enterApp:", e); }
    return out;
  };
})();
/* =========================================================================
   BẢN VÁ 2: XOÁ CHAT NHÓM • ĐỌC ẨN CHỈ DÀNH CHO ADMIN • MƯỢT & KHÔNG NHẤP NHÁY
   ========================================================================= */
(() => {
  const $ = (id) => document.getElementById(id);
  const isBoss = () => !!(window.session && window.session.role === "admin");
  const toast = (m) => (window.showToast ? window.showToast(m) : console.log(m));

  /* ---------------------------------------------------- 1. GỠ BỎ CHAT NHÓM */
  const noop = () => {};
  [
    "loadGroups", "openGroupChat", "closeGroupChat", "sendGroupChat",
    "openCreateGroupModal", "confirmCreateGroup", "openGroupManageModal",
    "uploadGroupAvatar", "spyGroupChat", "kickGroupMember", "disbandGroup",
    "addGroupMember",
  ].forEach((fn) => { window[fn] = noop; });
  window.currentGroupChat = "";

  /* --------------------------------------- 2. ĐIỀU PHỐI KÊNH CHAT (2 KÊNH) */
  window.openChatChannel = (type) => {
    if (type === "group") type = "global";
    if (type === "spy" && !isBoss()) type = "private";

    const btns = { global: "btn-chat-global", private: "btn-chat-private", spy: "btn-chat-spy" };
    Object.keys(btns).forEach((k) => {
      const b = $(btns[k]);
      if (!b) return;
      const on = k === type;
      b.style.background = on ? "var(--pink)" : "var(--border)";
      b.style.color = on ? "#fff" : (k === "spy" ? "#dc3545" : "var(--text)");
    });

    const spyBtn = $("btn-chat-spy");
    if (spyBtn) spyBtn.classList[isBoss() ? "remove" : "add"]("hidden");

    ["chat-global-zone", "chat-private-zone", "admin-spy-zone"].forEach((z) => {
      const el = $(z); if (el) el.classList.add("hidden");
    });

    if (type === "global") {
      const g = $("chat-global-zone");
      if (g) { g.classList.remove("hidden"); window.loadGlobalChat(); }
      return;
    }

    if (type === "spy") {
      const s = $("admin-spy-zone");
    ...
    // private
    const p = $("chat-private-zone");
    if (!p) return;
    p.classList.remove("hidden");
    const hasConvo = !!window.currentPrivateConvo;
    const sView = $("private-search-view");
    const cArea = $("private-chat-area");
    if (sView) sView.classList[hasConvo ? "add" : "remove"]("hidden");
    if (cArea) cArea.classList[hasConvo ? "remove" : "add"]("hidden");
    if (!hasConvo && typeof window.renderRecentChats === "function") window.renderRecentChats();
  };

  /* ------------------------------- 3. TRUNG TÂM ĐỌC ẨN — CHỈ ADMIN, MƯỢT   */
  window.loadAdminSpy = () => {
    if (!isBoss()) {
      const s = $("admin-spy-zone"); if (s) s.classList.add("hidden");
      return;
    }
    if (window._spyListRef) { try { window._spyListRef.off(); } catch (e) {} }
    window._spyListRef = window.db.ref("chat_streaks");
    window._spyListRef.on("value", (snap) => {
      const list = $("admin-convo-list");
      if (!list) return;
      let html = "";
      snap.forEach((child) => {
        const convoId = child.key;
        if (!convoId || convoId.startsWith("grp_") || !convoId.includes("_")) return;
        const ids = convoId.split("_");
        const nm = (i) => (window.allUsersMap && window.allUsersMap[i] && window.allUsersMap[i].name) || i;
        html += `<button class="spy-convo-item card shadow-lux" style="text-align:left;width:100%;border:none;padding:14px;cursor:pointer;border-left:4px solid #dc3545;background:var(--card);"
          onclick="window.spyPrivateChat('${ids[0]}','${ids[1]}')">
          <div style="font-weight:bold;color:var(--pink);">${window.escapeHTML(String(nm(ids[0])))} 💬 ${window.escapeHTML(String(nm(ids[1])))}</div>
          <div style="font-size:11px;color:#888;">${ids[0].toUpperCase()} &amp; ${ids[1].toUpperCase()}</div>
        </button>`;
      });
      // chỉ ghi lại DOM khi nội dung thật sự đổi -> hết nhấp nháy
      const next = html || '<p style="text-align:center;color:#888;">Chưa có hội thoại nào.</p>';
      if (list.dataset.sig !== next) { list.dataset.sig = next; list.innerHTML = next; }
    });
  };

  window.spyPrivateChat = (id1, id2) => {
    if (!isBoss()) return toast("🚫 Chỉ Admin mới được đọc ẩn");
    const convoId = typeof window.getConvoId === "function"
      ? window.getConvoId(id1, id2) : [id1, id2].sort().join("_");

    window.isSpying = true;
    window.currentPrivateConvo = "";
    const s = $("admin-spy-zone"); if (s) s.classList.add("hidden");
    const pz = $("chat-private-zone"); if (pz) pz.classList.remove("hidden");
    const sv = $("private-search-view"); if (sv) sv.classList.add("hidden");
    const ca = $("private-chat-area"); if (ca) ca.classList.remove("hidden");
    const iz = $("private-chat-input-zone"); if (iz) iz.classList.add("hidden");
    const ti = $("private-chat-title");
    if (ti) ti.innerText = "🕵️ Đọc ẩn: " + id1.toUpperCase() + " & " + id2.toUpperCase();
    const nick = $("nickname-chat-btn"); if (nick) nick.classList.add("hidden");
    const notice = $("stranger-message-notice");
    if (notice) {
      notice.classList.remove("hidden");
      notice.innerHTML = "Chế độ ĐỌC ẨN — đối phương không thấy bạn online, không mất dấu 'chưa đọc'.";
    }

    const box = $("private-chat-box");
    if (box) box.innerHTML = '<div style="text-align:center;color:#888;margin-top:20px;">Đang tải…</div>';

    if (window.currentChatRef) { try { window.currentChatRef.off(); } catch (e) {} }
    window.currentChatRef = window.db.ref("chat/private/" + convoId).limitToLast(80);
    ...
    window.currentChatRef.on("child_added", (snap) => {
      ...
      const stick = atBottom() || box.childElementCount === 0;
      box.insertAdjacentHTML("beforeend", window.renderMessage(snap.val(), snap.val().id === id1, snap.key, "private", convoId));
      if (stick) box.scrollTop = box.scrollHeight;
    });
    window.currentChatRef.on("child_changed", (snap) => {
      const el = $("msg-" + snap.key);
      if (el) el.outerHTML = window.renderMessage(snap.val(), snap.val().id === id1, snap.key, "private", convoId);
    });
    window.currentChatRef.on("child_removed", (snap) => {
      const el = $("msg-" + snap.key); if (el) el.remove();
    });
    setTimeout(() => { if (box && first) box.innerHTML = '<div style="text-align:center;color:#888;margin-top:20px;">Chưa có tin nhắn.</div>'; }, 1500);
  };

  /* -------------------------------- 4. THOÁT CHAT RIÊNG / THOÁT ĐỌC ẨN     */
  window.closePrivateChat = () => {
    const wasSpy = window.isSpying;
    if (window.currentChatRef) { try { window.currentChatRef.off(); } catch (e) {} }
    window.currentChatRef = null;
    window.currentPrivateConvo = "";
    window.isSpying = false;
    const ca = $("private-chat-area"); if (ca) ca.classList.add("hidden");
    const iz = $("private-chat-input-zone"); if (iz) iz.classList.remove("hidden");
    const nick = $("nickname-chat-btn"); if (nick) nick.classList.remove("hidden");
    const notice = $("stranger-message-notice"); if (notice) notice.classList.add("hidden");
    window.openChatChannel(wasSpy && isBoss() ? "spy" : "private");
  };

  /* -------------------------------- 5. LỌC DANH SÁCH ĐỌC ẨN (không nháy)   */
  window.filterAdminSpy = (el) => {
    const kw = (el && el.value ? el.value : "").toLowerCase().trim();
    document.querySelectorAll("#admin-convo-list .spy-convo-item").forEach((it) => {
      it.style.display = it.innerText.toLowerCase().includes(kw) ? "" : "none";
    });
  };

  /* -------------------------------- 6. GẮN LẠI SAU KHI VÀO APP             */
  const prevEnter = window.enterApp;
  window.enterApp = function () {
    const out = prevEnter ? prevEnter.apply(this, arguments) : undefined;
    setTimeout(() => {
      try {
        const spyBtn = $("btn-chat-spy");
        if (spyBtn) spyBtn.classList[isBoss() ? "remove" : "add"]("hidden");
        ...
        window.openChatChannel("global");
      } catch (e) { console.warn("[patch2]", e); }
    }, 300);
    return out;
  };
})();
...
/* =========================================================================
   PHẦN AN TOÀN DỮ LIỆU (chống mất dữ liệu) — chạy sau cùng
   Tự động sao lưu vào node "backups/<mốc thời gian>" TRƯỚC khi xoá diện rộng.
   ========================================================================= */
(function () {
  "use strict";
  ...
  // Sao lưu 1 danh sách node sang backups/<ts>/...
  window.backupNodes = async function (nodes, label) {
    if (!window.db) throw new Error("Chưa kết nối cơ sở dữ liệu");
    const key = (label || "backup") + "_" + stamp();
    const data = {};
    for (const node of nodes) {
      try {
        const snap = await window.db.ref(node).once("value");
        const val = snap.val();
        if (val !== null && val !== undefined) data[node.replace(/\//g, "__")] = val;
      } catch (e) {
        console.warn("[backup] không đọc được", node, e);
      }
    }
    await window.db.ref("backups/" + key).set({
      createdAt: typeof window.now === "function" ? window.now() : Date.now(),
      by: (window.session && window.session.id) || "unknown",
      label: label || "backup",
      data: data,
    });
    return key;
  ...
  // Tải toàn bộ dữ liệu ra file JSON trên máy (nút "xuất dữ liệu")
  window.exportAllData = async function () {
    if (!window.isAdminLevel || !window.isAdminLevel()) {
      return window.showCustomAlert("KHÔNG CÓ QUYỀN", "Chỉ quản lý mới được xuất dữ liệu!", "🔒");
    }
    const nodes = ["users", "user_passwords", "grades", "grade_logs", "tracking", "friends",
      "chat", "chat_streaks", "groups", "inbox", "replies", "announcements",
      "attendance_data", "attendance_weeks", "nicknames", "deleted_logs", "lock_logs", "config"];
    try {
      const dump = {};
      for (const n of nodes) {
        const snap = await window.db.ref(n).once("value");
        dump[n] = snap.val();
      }
      const blob = new Blob([JSON.stringify(dump, null, 2)], { type: "application/json" });
      const a = document.createElement("a");
      a.href = URL.createObjectURL(blob);
      a.download = "sao-luu-lop-hoc-" + stamp() + ".json";
      document.body.appendChild(a); a.click(); a.remove();
      setTimeout(() => URL.revokeObjectURL(a.href), 4000);
      if (window.showToast) window.showToast("✅ Đã tải file sao lưu về máy!");
    } catch (e) {
      window.showCustomAlert("LỖI SAO LƯU", e.message, "❌");
    }
  };

  // Bọc lệnh "ném bom": sao lưu trước khi xoá
  const HARD_RESET_NODES = ["users", "user_passwords", "grades", "grade_logs", "tracking", "friends",
    "chat", "chat_streaks", "groups", "inbox", "replies", "announcements",
    "attendance_data", "attendance_weeks", "deleted_logs", "lock_logs", "config"];

  const prevHardReset = window.executeHardReset;
  window.executeHardReset = async function () {
    if (!window.session || !window.isAdminLevel()) {
      return window.showCustomAlert("LỖI BẢO MẬT", "Hành động bị từ chối!", "🚨");
    }
    try {
      const key = await window.backupNodes(HARD_RESET_NODES, "hard_reset");
      if (window.showToast) window.showToast("💾 Đã sao lưu trước khi xoá: " + key);
    } catch (e) {
      return window.showCustomAlert("DỪNG LẠI", "Không sao lưu được nên KHÔNG xoá dữ liệu: " + e.message, "🛑");
    }
    return prevHardReset ? prevHardReset.apply(this, arguments) : undefined;
  };

  const prevClearLogs = window.clearGradeLogs;
  window.clearGradeLogs = function () {
    if (!window.isAdminLevel || !window.isAdminLevel()) {
      return window.showCustomAlert("CẢNH BÁO", "Chỉ quản lý mới có quyền dọn lịch sử sửa điểm!", "🚨");
    }
    window.backupNodes(["grade_logs"], "grade_logs")
      .then(() => prevClearLogs && prevClearLogs.apply(window, []))
      .catch((e) => window.showCustomAlert("DỪNG LẠI", "Không sao lưu được: " + e.message, "🛑"));
  };

  // Phục hồi từ bản sao lưu (chỉ Boss)
  window.restoreBackup = function (key) {
    if (!window.isBoss || !window.isBoss()) {
      return window.showCustomAlert("KHÔNG CÓ QUYỀN", "Chỉ Boss mới được phục hồi dữ liệu!", "🔒");
    }
    if (!key) return;
    window.showCustomConfirm("PHỤC HỒI DỮ LIỆU", "Ghi dữ liệu từ bản sao lưu " + key + " trở lại hệ thống?", async () => {
      try {
        const snap = await window.db.ref("backups/" + key + "/data").once("value");
        const data = snap.val() || {};
        for (const k of Object.keys(data)) {
          await window.db.ref(k.replace(/__/g, "/")).set(data[k]);
        }
        window.showCustomAlert("THÀNH CÔNG", "Đã phục hồi dữ liệu từ " + key, "✅");
      } catch (e) {
        window.showCustomAlert("LỖI PHỤC HỒI", e.message, "❌");
      }
    });
  };

  // Cảnh báo khi đang gõ dở mà thoát trang (tránh mất nội dung)
  window.addEventListener("beforeunload", function (e) {
    const dirty = Array.from(document.querySelectorAll("textarea, input[type=text]"))
      .some((el) => !el.closest(".hidden") && el.value && el.value.trim().length > 40 && el.dataset.noWarn !== "1");
