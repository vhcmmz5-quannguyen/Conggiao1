...
const adminTabs = ['nav-myprofile', 'nav-connect', 'nav-chat', 'nav-personal', 'nav-attendance', 'nav-manage', 'nav-rules', 'nav-tracking', 'nav-avatar', 'nav-users', 'nav-branding', 'nav-settings', 'nav-clear-data'];
const hsTabs = ['nav-myprofile', 'nav-connect', 'nav-chat', 'nav-personal', 'nav-rules', 'nav-settings']; 
const gvTabs = ['nav-myprofile', 'nav-connect', 'nav-chat', 'nav-attendance', 'nav-manage', 'nav-rules', 'nav-users', 'nav-settings'];
...
// =======================================================
(function() {
    // 1. Cấu hình khóa bí mật của bạn (Dán API Key vào đây)
    const GEMINI_API_KEY = "AIzaSyBAAxKAhglHrUXRgNmaSs6ZLjunBumbdFM"; 
    // SỬA LỖI: "gemini-1.5-flash" đã bị Google khai tử (API trả về lỗi 404)
    // khiến AI luôn báo "Gặp sự cố xử lý câu hỏi". Nâng cấp lên model mới nhất.
    ...
    function khởiTạoTrợLýAI() {
        ...
        async function requestGeminiAI(userPrompt) {
            if (GEMINI_API_KEY === "ĐIỀN_KEY_API_CỦA_BẠN_VÀO_ĐÂY" || !GEMINI_API_KEY) {
                removeLoading();
                pushMessageToScreen("❌ Hệ thống chưa cấu hình Key API trong file JavaScript.");
                return;
            }
            ...
            const systemRule = {
                parts: [{ text: `Bạn là trợ lý AI thông minh tích hợp trên ứng dụng "Lớp Học Công Giáo". Bạn nói chuyện với học viên cực kỳ thân thiện, lễ phép, sử dụng các icon vui tươi và từ ngữ sư phạm Công giáo. Bạn giúp giải đáp các thắc mắc về Giáo lý, bài học và Kinh Thánh. Người đang chat với bạn tên là: ${getAppUserName()}.` }]
            };
            ...
            try {
                ...
                // SỬA LỖI: Truy xuất an toàn bằng optional chaining, tránh crash
                // khi API trả về cấu trúc thiếu (trước đây data.candidates[0].content
                // .parts[0].text sẽ ném lỗi TypeError làm bung bẫy lỗi toàn cục)
                ...
                if (botReply) {
                    ...
                    // SỬA LỖI: Hiện lý do lỗi thật từ Google thay vì thông báo chung chung
                    // Xóa lượt hỏi lỗi khỏi lịch sử để không làm hỏng các lượt hỏi sau
                    aiChatHistory.pop();
                    pushMessageToScreen("❌ Máy chủ AI báo lỗi: " + data.error.message, false);
                } else {
                    aiChatHistory.pop();
                ...
                removeLoading();
                aiChatHistory.pop();
                pushMessageToScreen("❌ Lỗi đường truyền mạng hoặc Key API chưa chính xác.", false);
            }
        }
