...
window.handleLogout = () => {
    ...
    if (typeof window.clearPaymentPinCache === 'function') window.clearPaymentPinCache();
    try {
...
const adminTabs = ['nav-myprofile', 'nav-connect', 'nav-chat', 'nav-personal', 'nav-attendance', 'nav-payments', 'nav-manage', 'nav-rules', 'nav-tracking', 'nav-avatar', 'nav-users', 'nav-branding', 'nav-settings', 'nav-clear-data'];
...
window.openUserActionMenu = (id, name, pass, isLocked, targetRole) => {
    ...
    if (isGv && (isTargetBoss || isTargetGv)) { 
...
window.loadAnnouncements = () => {
    ...
    if (!window.db) {
        console.warn("Firebase chưa sẵn sàng để tải thông báo!");
        return;
    }

    // Đọc vai trò (role) của tài khoản đang đăng nhập (admin / gv / hs)
    // Nếu chưa đăng nhập thì mặc định coi như học sinh 'hs' để bảo mật
    const currentRole = window.session?.role || "hs"; 

    // Lắng nghe danh sách thông báo từ Firebase Realtime Database
    window.db.ref('announcements').on('value', (snapshot) => {
        displayEl.innerHTML = ""; // Xóa sạch danh sách cũ để nạp mới
        
        const data = snapshot.val();
        if (!data) {
            displayEl.innerHTML = `<div style="text-align:center; color:var(--text-light); padding:30px; font-style:italic; font-size:13px;">📢 Chưa có thông báo nào được đăng.</div>`;
            return;
        }

        // Chuyển dữ liệu Object từ Firebase thành mảng để sắp xếp thời gian
        const list = [];
        Object.keys(data).forEach(key => {
            list.push({ id: key, ...data[key] });
        });
        
        // Sắp xếp: Thông báo mới nhất luôn nhảy lên trên cùng
        list.sort((a, b) => b.timestamp - a.timestamp);

        // Duyệt qua từng thông báo để kiểm tra quyền xem
        list.forEach(item => {
            const target = item.target || "all";

            // 🛑 THUẬT TOÁN BỘ LỌC BẢO MẬT:
            // 1. Nếu là Admin: Được quyền xem toàn bộ thông báo không giới hạn.
            // 2. Nếu không phải Admin: Chỉ được xem nếu thông báo đó gửi cho 'all' HOẶC trùng với role của mình.
            if (currentRole !== 'admin' && target !== 'all' && target !== currentRole) {
                return; // Bỏ qua, không cho hiển thị thông báo này lên màn hình
            }

            // Tạo nhãn (Badge) xinh xắn hiển thị phạm vi gửi thông báo
            let targetBadge = '';
            ...
            if (target === 'hs') targetBadge = '<span style="background:rgba(40,167,69,0.1); color:#1b5e20; padding:2px 6px; border-radius:6px; font-size:11px; font-weight:bold;">🎓 Học Sinh</span>';
            if (target === 'gv') targetBadge = '<span style="background:rgba(255,193,7,0.15); color:#b78103; padding:2px 6px; border-radius:6px; font-size:11px; font-weight:bold;">👨‍🏫 Giáo Viên</span>';

            // Chuyển đổi Timestamp thành ngày giờ dễ đọc
            ...
            // Quyền đặc biệt: Chỉ Admin mới có nút XÓA thông báo trực tiếp
            ...
            // Tạo khung HTML của từng thẻ thông báo (giao diện bo góc đồng bộ css project của bạn)
            const cardHtml = `
...
// 1. Mở Modal đổi PIN
window.openChangePinModal = () => { if (typeof window.clearPaymentPinCache === 'function') window.clearPaymentPinCache();
...
window.openAdminPasswordModal = () => {
    if (window.session.role !== 'admin') return window.showCustomAlert("🚫 Chỉ Boss mới được dùng tính năng này!");
...
window.renderTrackingTable = (snap) => {
    ...
    let userList = [];
    for (let i in d) { userList.push({ id: i, ...d[i] }); }
    userList.sort((a, b) => {
...
window.loadAttendanceSessions = () => {
    if (!window.session || (window.session.role !== 'admin' && window.session.role !== 'gv')) return;
...
window.openAttendanceWeek = (id, name) => {
    ...
    window.db.ref('attendance_weeks/' + id).on('value', s => {
        ...
        if (btnLock) {
            ...
            btnLock.innerHTML = window.isAttSessionLocked ? (window.session.role === 'admin' ? "🔓 MỞ KHÓA" : "🔒 ĐÃ CHỐT") : "🔒 CHỐT SỔ";
            btnLock.style.background = window.isAttSessionLocked ? (window.session.role === 'admin' ? "#4CAF50" : "#888") : "#FF9800";
        }
        if (btnDel) {
            if (window.session.role === 'admin' || (!window.isAttSessionLocked && window.session.role === 'gv')) btnDel.classList.remove('hidden');
...
window.openMarkMenu = (uid, name) => {
    if (window.isAttSessionLocked && window.session.role !== 'admin') {
...
window.confirmSaveScore = async () => {
    if (!window.requireRole(['admin', 'gv'], 'sửa điểm')) return;
    ...
    try {
        ...
        await window.db.ref('grade_logs').push({ studentId: id, term: `hk${term}`, editorId: window.session.id, editorName: window.session.name, score, time: firebase.database.ServerValue.TIMESTAMP });
        window.toggleModal('score-modal', false);
...
// ==========================================
// TAB ĐÓNG TIỀN - Thiết kế mới v2 (2 lớp xác nhận + hoàn tác)
// ==========================================
window.paymentRef = null;
window.paymentState = { students: [], payments: {}, filter: 'all', search: '', undo: null, undoTimer: null };

window.loadPayments = () => {
    if (!window.requireRole(['admin', 'gv'], 'xem danh sách đóng tiền')) return;
    ...
    Promise.all([window.db.ref('users').once('value'), window.db.ref('payments').once('value')]).then(([usersSnap, paymentsSnap]) => {
        ...
        window.paymentRef.on('child_changed', snap => { window.paymentState.payments[snap.key] = snap.val(); window.renderPayments(); });
        window.paymentRef.on('child_added', snap => { window.paymentState.payments[snap.key] = snap.val(); window.renderPayments(); });
...
window.renderPayments = () => {
    ...
    if (!shown.length) { list.innerHTML = '<div class="card payment-empty">Không có học sinh nào phù hợp.</div>'; return; }
...
// LỚP 1: xác nhận trước khi đổi
window.requestPaymentToggle = (studentId) => {
    if (!window.requireRole(['admin', 'gv'], 'cập nhật đóng tiền')) return;
    ...
    // BƯỚC 1: Hộp thoại Có / Không đơn giản
    const askPin = () => window.askAdminPinForPayment(studentId, nextPaid, wasPaid, s);
    if (typeof window.showCustomConfirm === 'function') {
        window.showCustomConfirm(title, msg, askPin);
    } else if (window.confirm(msg)) {
        askPin();
    }
};

// ĐÃ NÂNG CẤP: Cache PIN 15 phút trong bộ nhớ (chỉ trong phiên, không lưu localStorage)
// → Nhập PIN 1 lần, sau đó thao tác nhanh như phần mềm kế toán chuyên nghiệp.
// Tự xoá cache khi: hết 15 phút, logout, đổi PIN, rời tab đóng tiền quá 30 phút.
window.__paymentPinCache = { pin: null, expiresAt: 0 };
window.PAYMENT_PIN_TTL_MS = 15 * 60 * 1000; // 15 phút

window.clearPaymentPinCache = () => {
    window.__paymentPinCache = { pin: null, expiresAt: 0 };
};

window.askAdminPinForPayment = (studentId, nextPaid, wasPaid, s) => {
    const now = Date.now();
    const cache = window.__paymentPinCache;

    // Nếu PIN còn hiệu lực → xác thực nhanh với server rồi commit, không hỏi lại
    if (cache.pin && cache.expiresAt > now) {
        return window.db.ref('config/clearPin').once('value').then(snap => {
            const realPin = snap.val() || "654321";
            if (String(cache.pin) === String(realPin)) {
                // Gia hạn thêm 15 phút mỗi lần dùng thành công (sliding session)
                cache.expiresAt = now + window.PAYMENT_PIN_TTL_MS;
                window.commitPayment(studentId, nextPaid, wasPaid);
            } else {
                // PIN đã bị đổi phía server → xoá cache, hỏi lại
                window.clearPaymentPinCache();
                window.showCustomAlert('PHIÊN PIN HẾT HẠN', 'Mã PIN đã thay đổi, vui lòng nhập lại.', '🔐');
                window.askAdminPinForPayment(studentId, nextPaid, wasPaid, s);
            }
        }).catch(err => {
            window.showCustomAlert('LỖI KẾT NỐI', 'Không xác thực được PIN: ' + err.message, '❌');
        });
    }

    // Chưa có cache hoặc hết hạn → hỏi PIN 1 lần
    const remainMin = Math.round(window.PAYMENT_PIN_TTL_MS / 60000);
    const pin = window.prompt(
        `🔐 NHẬP MÃ PIN ADMIN\n\n` +
        `Sau khi nhập đúng, bạn sẽ không phải nhập lại trong ${remainMin} phút.\n\n` +
        `Thao tác đầu tiên: ${nextPaid ? 'đánh dấu ĐÃ ĐÓNG' : 'chuyển về CHƯA ĐÓNG'} cho "${s.name}"`
    );
    if (pin === null) return;
    if (!pin) {
        return window.showCustomAlert('THIẾU MÃ PIN', 'Vui lòng nhập mã PIN admin.', '⚠️');
    }
    window.db.ref('config/clearPin').once('value').then(snap => {
        const realPin = snap.val() || "654321";
        if (String(pin) === String(realPin)) {
            window.__paymentPinCache = { pin: String(pin), expiresAt: Date.now() + window.PAYMENT_PIN_TTL_MS };
            if (typeof window.showToast === 'function') {
                window.showToast(`🔓 Đã mở khoá thao tác đóng tiền trong ${remainMin} phút`, true);
            }
            window.commitPayment(studentId, nextPaid, wasPaid);
        } else {
            window.showCustomAlert('SAI MÃ PIN', 'Mã PIN admin không chính xác! Hành động bị từ chối.', '🚫');
        }
    }).catch(err => {
        window.showCustomAlert('LỖI KẾT NỐI', 'Không xác thực được PIN: ' + err.message, '❌');
    });
...
// Ghi Firebase (đã bỏ toast hoàn tác 8 giây)
window.commitPayment = async (studentId, nextPaid, prevPaid) => {
    const row = document.getElementById(`payment-${studentId}`);
    const btn = row?.querySelector('.pay-toggle');
    if (btn) { btn.disabled = true; btn.textContent = '⏳ Đang lưu...'; }
    try {
        await window.db.ref(`payments/${studentId}`).set({
            status: nextPaid ? 'paid' : 'unpaid',
            updatedBy: window.session.id,
            updatedByName: window.session.name,
            updatedAt: firebase.database.ServerValue.TIMESTAMP
        });
        if (row) { row.classList.add('is-flash'); setTimeout(() => row.classList.remove('is-flash'), 600); }
        const s = window.paymentState.students.find(x => x.id === studentId);
        window.showCustomAlert(
            nextPaid ? 'ĐÃ ĐÓNG TIỀN' : 'CHƯA ĐÓNG TIỀN',
            `Đã cập nhật trạng thái cho:\n"${s ? s.name : studentId}"`,
            nextPaid ? '✅' : '⏳'
        );
    } catch (error) {
        window.showCustomAlert('KHÔNG THỂ LƯU', error.message, '❌');
        window.renderPayments();
    }
};
...
// Vá memory leak: rời tab thì tắt listener + xoá PIN cache sau 30 phút không dùng
window.unloadPayments = () => {
    if (window.paymentRef) { window.paymentRef.off(); window.paymentRef = null; }
    window.hideUndoToast();
    // Không xoá PIN cache ngay — user có thể quay lại tab. Nhưng đặt hẹn 30 phút.
    if (window.__paymentPinCacheClearTimer) clearTimeout(window.__paymentPinCacheClearTimer);
    window.__paymentPinCacheClearTimer = setTimeout(() => window.clearPaymentPinCache(), 30 * 60 * 1000);
};
...
window.checkAndStartPrivateChat = (targetId, targetName, allowPrivate) => {
    if (!allowPrivate && window.session.role !== 'admin') return window.showCustomAlert('NGƯỜI DÙNG ĐÃ TẮT CHAT', 'Không thể gửi tin nhắn riêng cho tài khoản này.', '🔕');
